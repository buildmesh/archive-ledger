    ...
    pub fn run_at_most(&self, limit: Option<usize>) -> Result<ScanResult> {
        ...
        for item in discovery.by_ref() {
            match item {
                ...
                DiscoveryItem::Symlink(_) => summary.symlinks += 1,
        ...
        coverage_partial |= self.confirm_source_unchanged(first_namespace, &mut summary)?;
        if coverage_partial {
            self.complete_scan("partial", &summary, None)?;
            return Ok(ScanResult {
                status: ScanStatus::Partial,
...
mod tests {
    ...
    fn source_change_after_candidate_generation_keeps_negatives_inert() {
        ...
        let mut summary = ScanSummary::default();
        assert!(concurrent
            .confirm_source_unchanged(first_namespace, &mut summary)
            .unwrap());
